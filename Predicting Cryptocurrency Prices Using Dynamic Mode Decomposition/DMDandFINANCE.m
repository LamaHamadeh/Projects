...
%check the eigenvalues that holds the most variance percentage
sigma = diag(S);
figure;
subplot(1,2,1)
...
% hold on
% %show the dynamics of the reconstructrd DMD mode for the first crypto
% subplot(1,2,2)
% plot(t,real(X_DMD(1,:)))
% axis square
% xlabel('Time')
...
%Using time delay embeddings to extract 'the' linear dynamics space 
% (Koopman space)
% Delay coordinates refer to an augmented vector obtained by stacking the 
% states at the current time with copies of the states at future times.
...
%apply SVD and rank truncation on H
[U,S,V] = svd(H,'econ');
%check the eigenvalues that holds the most variance percentage
sigma = diag(S);
figure;
plot(sigma/sum(sigma),'ko','LineWidth',2)
axis square
xlabel('$r$','interpreter','latex')
ylabel('$\displaystyle{\sigma_i/\sum_{i=1}^{N} \sigma_i}$','interpreter','latex')
title('Eigenvalues of Hankel Matrix')
%check the time dynamics eigenvectors
figure;
...
axis square
hold on
plot(V(:,3),'g','LineWidth',2)
xlabel('$t$','interpreter','latex')
axis square
hold on
plot(V(:,4),'y','LineWidth',2)
xlabel('$t$','interpreter','latex')
axis square
ylabel('$V$','interpreter','latex')


%Apply the DMD on Hankel matrix H
% X1 = H(:,1:end-1);
% X2 = H(:,2:end);
