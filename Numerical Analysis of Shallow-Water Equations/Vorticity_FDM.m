...
for j=1:Nx
    ...
    e4(Nx*j)=1; % overwirte every Nx^th value with one
end
e3(2:N,1)=e2(1:N-1,1); 
e3(1,1)=e2(N,1); % set boundary
e5(2:N,1)=e4(1:N-1,1); 
e5(1,1)=e4(N,1); % set boundary
...
A(1,1) = 2; %this should be set instead of A(1,1)=-4, 
%otherwise ode45 goes to infinity
A = A/(delta^2);
...
%ode45(explicit Runge-Kutta (4,5) formula, the Dormand-Prince pair)
