...
%Case 3: in this case, you have a horizontal displacement. Here, the mass 
% is released off-center to produce motion in the x-y plane as well as 
% the z-direction. Thus there is both a pendulum motion and simple harmonic
% oscillations. See what the PCA tells us about the system. 
% (camN_3.mat where N=1,2,3)
...
[row1,col1,rgb1,frame1] = size(ds1.vidFrames1_3);
[row2,col2,rgb2,frame2] = size(ds2.vidFrames2_3);
[row3,col3,rgb3,frame3] = size(ds3.vidFrames3_3);
...
X1_3 = zeros(1,frame1); % Store object's x location
Y1_3 = zeros(1,frame1); % Store object's y location

for i = 1:frame1
    % Chnage each frame's type to a "double" instead of its uint8
    % format
    img1(:,:,:,i) = double(rgb2gray(ds1.vidFrames1_3(:,:,:,i)));
...
%initialise x and y location matrices
X2_3 = zeros(1,frame2); % Store object's x location
Y2_3 = zeros(1,frame2); % Store object's y location

for i = 1:frame2
    % Chnage each frame's type to a "double" instead of its uint8
    % format
    img2(:,:,:,i) = double(rgb2gray(ds2.vidFrames2_3(:,:,:,i)));
    % Image Cropping: isolate the paint can in the video by making 
    % areas around the can black, i.e., pixels values equal to zero.
    for r = 1:row2
        for c = 1:col2
...
%initialise x and y location matrices
X3_3 = zeros(1,frame3); % Store object's x location
Y3_3 = zeros(1,frame3); % Store object's y location

for i = 1:frame3
    % Chnage each frame's type to a "double" instead of its uint8
    % format
    img3(:,:,:,i) = double(rgb2gray(ds3.vidFrames3_3(:,:,:,i)));
    % Image Cropping: isolate the paint can in the video by making 
    % areas around the can black, i.e., pixels values equal to zero.
    for r = 1:row3
        for c = 1:col3
...
%data matrix 6x226
X = [X1_3(1:frame3) ; Y1_3(1:frame3) ; 
    X2_3(1:frame3) ; Y2_3(1:frame3) ;
    X3_3(1:frame3) ; Y3_3(1:frame3)];
...
%In this test, the mass was pushed horizontally as well as vertically so
%that it is both bobbing up and down and swaying from left to right. Thus
%we would expect PCA to discover two important modes (or perhaps three if
%the bucket was not pushed perfectly horizontally).
%The plots show clear indication of horizontal and verical oscillations.
%The oscillation on the third diemnsion is showing but not as strong as the
%osicllations in the x and y dimensions.
%The percentage variance explained is more distributed among the PCA modes.
%The rest of the variance observed is attributed to noise. 
%------------------
